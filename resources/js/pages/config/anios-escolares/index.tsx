...
export default function AcademicYears({ academicYears }: Props) {
    const [yearsState, setYearsState] = useState<AcademicYear[]>(
        academicYears.data,
    );
    ...
    const [currentPage, setCurrentPage] = useState<number>(
        academicYears.current_page || initialPage,
    );
    const [limitPage, setLimitPage] = useState<number>(
        academicYears.per_page || initialPerPage,
    );
    ...
    useEffect(() => {
        setYearsState(academicYears.data);
        setLoading(false);
        if (!urlParams.get('page') && academicYears.current_page) {
            setCurrentPage(academicYears.current_page);
        }
        if (!urlParams.get('per_page') && academicYears.per_page) {
            setLimitPage(academicYears.per_page);
        }
    }, [academicYears]);
    ...
    const buttons: ToolButtonOptions[] = [
        {
            ...
            onClick: () => {
                handleSearch({
                    ...
                    offset: 0,
                    last_page: Math.ceil(
                        (academicYears.recordsTotal || 0) / limitPage,
                    ),
                });
            },
        },
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-4 p-4">
                ...
                <DataTable
                    ...
                    pagination={{
                        totalRecords: academicYears.recordsTotal,
